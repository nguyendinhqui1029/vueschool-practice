      ...
      <ul :class="listType" class="flex flex-col ml-4 mt-4 list-outside border-l-2 px-2 gap-6">
        <li v-for="(item) of optionsValueForm" :key="`content_list_display_${item}`" class="flex flex-col gap-3">
          <div v-if="languages.includes(LANGUAGE_CODE.EN)" class="flex gap-4 items-start">
            <img v-if="languages.length > 1" class="w-8 h-6 cursor-pointer"  src="https://www.flagpedia.net/data/flags/h80/us.png" alt="United States Flag" />
            <span>{{ item.en }}</span>
          </div>
          <div v-if="languages.includes(LANGUAGE_CODE.VI)" class="flex gap-4 items-start">
            <img v-if="languages.length > 1" class="w-8 h-6 cursor-pointer"  src="https://www.flagpedia.net/data/flags/h80/vn.png" alt="United States Flag" />
            <span>{{ item.vi }}</span>
          </div>
          <div v-if="languages.includes(LANGUAGE_CODE.KO)" class="flex gap-4 items-start">
            <img v-if="languages.length > 1" class="w-8 h-6 cursor-pointer"  src="https://www.flagpedia.net/data/flags/h80/kr.png" alt="United States Flag" />
            <span>{{ item.ko }}</span>
          </div>
        </li>
      </ul>
